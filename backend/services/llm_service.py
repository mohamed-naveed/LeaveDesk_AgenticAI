...
class LLMService:
    ...
    def _fallback_process_chat(self, text: str, context_str: str) -> dict:
        ...
        if is_manager:
            ...
            is_history_query = any(w in text_lower for w in ["history", "histroy", "histry", "past", "last", "previous", "applied", "request", "requests"])
        ...
        if any(w in text_lower for w in ["my details", "profile details", "my profile", "get my details", "show my details", "mt details", "get mt details", "profile info", "my info"]):
            profile_lines = []
            capture = False
            for line in context_str.split("\n"):
                if "Employee Profile:" in line:
                    ...
                    continue
                if capture:
                    if line.strip() == "" or "Employee Balances:" in line or "Company Leave Policy:" in line:
                        break
                    profile_lines.append(line)
            if profile_lines:
                return {
                    "intent": "general_inquiry",
                    "chat_response": "Here are your profile details:\n" + "\n".join(profile_lines)
                }
            return {
                "intent": "general_inquiry",
                "chat_response": "I couldn't find your profile details in the system."
            }
        ...
        if any(w in text_lower for w in ["history", "histroy", "histry", "last leave", "past leave", "applied", "previous", "leaves", "leave", "request", "requests"]):
            is_team_query = any(w in text_lower for w in ["team", "teammate", "teammates", "colleague", "colleagues", "others", "other"])
